(function (root) {
  'use strict';
  const MS=root.MarxSim=root.MarxSim||{},clone=value=>JSON.parse(JSON.stringify(value));
  MS.createGameState=function createGameState(scenario,seed){const data=clone(scenario),districts=data.districts.map(d=>Object.assign({land_cost:0.7,housing_capacity:400,base_rent:0.14,rent:d.base_rent||0.14,land_value:1,pollution:0.05,road_level:0,rail_level:0,service_level:0,resource_bonus:0,industry_bonus:0,tax_rate:0},d));return {version:MS.CONFIG.version,scenarioId:data.id,seed:Number(seed)>>>0,rngState:Number(seed)>>>0,tick:0,date:{year:data.startYear||MS.CONFIG.startYear,month:data.startMonth||MS.CONFIG.startMonth},districts,sectors:data.sectors,
  ...
  firms:data.firms.map(f=>Object.assign({active:true,employees:0,desired_employees:0,inventory:0,output:0,sales:0,revenue:0,profit:0,expected_demand:f.capacity*0.72,utilization:0,insolvency_months:0,age:0,trailing_sales:[],market_share:0,strike_intensity:0,working_hours:12},f)),
